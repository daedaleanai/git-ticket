...
func (c CreateTicketAction) FromValues(values url.Values) http_webui.ValidatedPayload {
	for k := range values {
	...
	c.Title = values.Get(keyTitle)
	c.Message = values.Get(keyMessage)
	c.Repo = values.Get(keyRepo)
	c.Workflow = values.Get(keyWorkflow)
