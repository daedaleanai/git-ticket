...
//
// Ex: "all(status(open), author(descartes)) sort(edit-asc)"
//
// Supported filter qualifiers and syntax are described in docs/queries.md
func Parse(query string) (*CompiledQuery, error) {
	p, err := NewParser(query)
	if err != nil {
		return nil, err
	}

	compiledQuery, err := p.Parse()
	if err != nil {
		return nil, err
	}
	fmt.Printf("%v\n", compiledQuery)
...
func parseColor(parser *Parser) (AstNode, error) {
	_, err := parser.parseDelimitedExpressionList()
	if err != nil {
		return nil, err
	}
	...
	// if strings.HasPrefix(lit, "ccb-pending-user:") {
