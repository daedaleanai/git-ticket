...
func promptUnknownRepositoryLabel(labelsMap map[config.Label]config.SingleLabelConfig, repo, id, title string) (string, error) {
		...
		for _, ticketId := range env.backend.AllBugsIds() {
			...
			if match := titleMatch.FindStringSubmatch(title); match != nil {
				...
				if _, ok := labels[config.Label(repo)]; !ok {
					selected, err := promptUnknownRepositoryLabel(labels, repo, ticketId.Human(), title)
